...
void PrintVector(vector<double>& vector);//打印向量
...
vector<double> gauss_equation_solving(vector<vector<double>>& A, vector<double>& b); //用列主元Gauss消去求解方程
...
vector<double> sign(vector<double>& w); //生成w的符号向量

int sign(double m); //m>0为1, m<0为-1, m=0为0

double InnerProduct(vector<double>& a, vector<double>& b);//计算向量内积

double VectorInfinityNorm(vector<double>& vec); //计算向量的无穷范数

double VectorOneNorm(vector<double>& vec); //计算向量的一范数

vector<double> UnitVectorGenerating(vector<double>& vec, int n); //生成无穷范数对应下标的单位向量

double MatrixOneNorm(int n, vector<vector<double>>& A);//计算矩阵的一范数

double MatrixInfinityNorm(vector<vector<double>>& matrix); //计算矩阵的无穷范数

vector<double> MatrixVectorMultiply(vector<vector<double>>& A, vector<double>& b);//计算矩阵A和向量b相乘得到的向量
...
void QRDecomposition(vector<vector<double>>& A, vector<double>& d); //计算QR分解

vector<vector<double>> HouseholderMatrix(vector<vector<double>>& A, vector<double>& d, int k); //生成Householder变换矩阵

vector<double> QR_equation_solving(vector<vector<double>>& A, vector<double>& b); //基于QR分解求解方程组
...
vector<vector<double>> MultiplyMatrices(vector<vector<double>>& matrix1, vector<vector<double>>& matrix2); //计算矩阵相乘

void ReduceMatrix(vector<vector<double>>& Q, int n); //将矩阵Q只保留前n列

vector<double> LS_proplem_solving(vector<vector<double>>& A, vector<double>& b); //求解线性最小二乘问题

double VectorTwoNorm(vector<double>& x); //计算向量的二范数
...
vector<vector<double>> MatrixAddition(vector<vector<double>> x, vector<vector<double>> y); // 返回两个矩阵之差
...
vector<vector<double>> intMatrix(int rows, int cols); // 生成整数矩阵

vector<vector<double>> randomMatrix(int rows, int cols); // 生成随机数矩阵

vector<vector<double>> idMatrix(int n); // 生成单位阵

void transpose(vector<vector<double>>& A); // 实现矩阵的转置

vector<vector<double>> Householder(vector<double>& v, double beta); // 生成Household矩阵

void bidiag(vector<vector<double>>& A, vector<vector<double>>& P, vector<vector<double>>& Q); // 实现m*n阶矩阵的二对角化

void zeroing2(vector<vector<double>>& A, double epsilon); //将矩阵中满足条件的对角元和次对角元置零

void pqq(vector<vector<double>>B, int& p, int& q); // 找到最大的p和最小的q

void givens(double a, double b, double& c, double& s); // 计算得到对于a和b的cos与sin值

vector<vector<double>> Givens(int n, int p, int q, double c, double s); // 由cos与sin值生成对应的givens变换矩阵

void GivensSVD(vector<vector<double>>& B, vector<vector<double>>& B22, int p, int i, vector<vector<double>>& U); // 若B22有对角元为0，用Givens变换把该行打成0

void WilkSVD(vector<vector<double>>& B, vector<vector<double>>& U, vector<vector<double>>& V);

double absmax(vector<vector<double>> A); // 返回矩阵绝对值最大的元素

void singluar(vector<vector<double>> A); // 从小到大输出奇异值